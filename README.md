...
### 镜像构建慢 / `go mod download` 超时

- **原因**：`go.mod` 要求 Go 1.25 时，若使用 `golang:1.24` 且 `GOTOOLCHAIN=auto`，会联网下载 toolchain（`proxy.golang.org`），国内常超时。
- **本项目 Dockerfile**：已使用 **`golang:1.25-alpine`** + **`GOTOOLCHAIN=local`**（只用镜像自带 Go），并默认 **`GOPROXY=https://goproxy.cn,direct`** 加速模块下载。
- **海外构建**：`docker build --build-arg GOPROXY=https://proxy.golang.org,direct .`
- 若拉不到 `golang:1.25-alpine`，可把 Dockerfile 第一行改为 `FROM golang:1.25-bookworm AS builder`（或当时官方已提供的 `1.25` 标签）。
